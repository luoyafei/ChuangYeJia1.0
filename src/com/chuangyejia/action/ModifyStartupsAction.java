...
@Component(value="modifyStartupsAction")
@Scope("prototype")
public class ModifyStartupsAction extends ActionSupport {
	/**
	 * 
	 */
	...
	private Startups startupsModify = null;
	
	public ModifyStartupsAction() {
			...
			if(!sd.checkData() || startupsModify == null || !startupsModify.getStartupsLeader().getUserId().equals(user.getUserId())) {
				...
				if(imgCanUse(img)) {
					...
					if(create_cover) {
						...
						startupsModify.setStartupsOperationStage(startups.getStartupsOperationStage());
						startupsModify.setStartupsAddress(startups.getStartupsAddress());
						startupsModify.setStartupsBrief(startups.getStartupsBrief());
						startupsModify.setStartupsDetail(startups.getStartupsDetail());
						if(startups.getStartupsVideo() != null && startups.getStartupsVideo().trim().hashCode() != 0) {
							startupsModify.setStartupsVideo(startups.getStartupsVideo());
						}
						
						
						if(ss.updateStartups(startupsModify)) {
							
							startupsId = startupsModify.getStartupsId();
							request.setAttribute("modifyS", true);
							return SUCCESS;
						} else {
